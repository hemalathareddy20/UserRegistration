...
echo "------------------------------------------"
read -p "enter a valid password(atleast 8 characters,1 number and 1 uppercase and exactly 1 special character) : " password
pat1="^(?=.{8,}$)(?=.*\d)(?=.*[A-Z])[a-zA-Z0-9]*[\@\#\^\!]*[a-zA-z0-9]*$"
if(( $password =~ $pat1 ))
then
     echo "$mobilenumber is a valid password"
else
